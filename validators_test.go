...
func TestRange(t *testing.T) {
	...
	goodValidatorsFloat := []Range{
		{Min{10.25}, Max{100.5}},
		ValidRangeFloat(10.25, 100.5),
	}
	...
	badValidatorsFloat := []Range{
		{Min{100}, Max{10}},
		ValidRangeFloat(100, 10),
	}
...
func runIPAddrTestfunc(t *testing.T, test_type int, ipaddr_list map[string]bool, msg_fmt string) {
	...
	for _, ip_test_list := range []IPAddr{{[]int{test_type}}, ValidIPAddr(test_type)} {
...
func TestMacAddr(t *testing.T) {
	...
	for _, mac_test_list := range []MacAddr{{}, ValidMacAddr()} {
...
func TestDomain(t *testing.T) {
	...
	for _, domain := range []Domain{{}, ValidDomain()} {
...
func TestPureTextNormal(t *testing.T) {
	...
	// normal
	for _, txt := range []PureText{{NORMAL}, ValidPureText(NORMAL)} {
...
func TestPureTextStrict(t *testing.T) {
	...
	// strict
	for _, txt := range []PureText{{STRICT}, ValidPureText(STRICT)} {
		performTests(txt, tests, t)
	}
}
...
func TestFilePathOnlyFilePath(t *testing.T) {
	...
	for _, filepath := range []FilePath{{ONLY_FILENAME}, ValidFilePath(ONLY_FILENAME)} {
...
func TestFilePathAllowRelativePath(t *testing.T) {
	...
	for _, filepath := range []FilePath{{ALLOW_RELATIVE_PATH}, ValidFilePath(ALLOW_RELATIVE_PATH)} {
		performTests(filepath, tests, t)
	}
}
